...
os.makedirs(out_dir, exist_ok=True)                     #create output directory (if it doesn't exist yet)
data = pd.read_csv(survey_path, float_precision='high') #read dhs/lsms_wealth_index.csv to pandas dataframe
#%%
    # convert URBAN_RURA column from string to int: 'U' (urban) => 1, 'R' (rural) => 0
    data.loc[:, 'URBAN_RURA'] = data['URBAN_RURA'].map({'U': 1, 'R': 0})

#%%
    use_years = set(range(2009, 2017+1))    # define range of required years
    digits = [str(i) for i in range(10)]    # define set of digits (0-9)
